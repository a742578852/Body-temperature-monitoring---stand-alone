	...
	for (let item of data) {
		...
		for (let i of item) {
			...
			if(i.type==='img'){
				...
				table += `<td style="width:${width};height:${height};color:${color};font-size:${fontSize};text-align:${textAlign}" colspan="${colspan}" rowspan="${rowspan}"><img src="/图片${imgCount}.jpg" width="${imgWidth}" height="${imgHeight}">(图片${imgCount}.jpg)</td>`;
				imgCount ++;
			}else{
				table += `<td style="width:${width};height:${height};color:${color};font-size:${fontSize};text-align:${textAlign}" colspan="${colspan}" rowspan="${rowspan}">${i.content}</td>`;
			}
		}
		table += `</tr>`;
	}
...
function exportFile(fileData,documentName,callback) {
	createDir(documentName,(dirEntry,path,name) => {
		dirEntry.getFile(`${documentName}.xls`, {
			create: true
		}, (fileEntry) => {
			fileEntry.createWriter((writer) => {
				...
				writer.onwrite = (e) => {
					...
					setTimeout(() => {
						...
						console.log(`导出成功，文件位置：${path}/${documentName}.xls`);
						callback(`${path}/${documentName}.xls`);
					}, 1000);
				};
				// 写入内容;
