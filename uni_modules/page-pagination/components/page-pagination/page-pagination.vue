		...
		<view class="page-con">
			<view class="page-scroll">
				<view class="page-scroll-child">
					...
					<!-- 下页 -->
					<view class="pag-btn next" :style="{color}" :class="[nowPage==pageNum?'btn-ban':'',btnText?'btn-text':'']" @click="clickNext">
						<text v-if="btnText">下页</text>
						<text v-else class="icon icon-right"></text>
					</view>
					<!-- 尾页 -->
					<view class="pag-btn end" :style="{color}" :class="[nowPage==pageNum?'btn-ban':'',btnText?'btn-text':'']" v-if="showAround && !numAround" @click="clickEnd">
	...
	export default {
		...
		created() {
			if(this.currentPage < 1){
				...
				this.nowPage = this.inputPage = this.pageNum;
			}else{
				this.nowPage = this.inputPage = this.currentPage;
			}
		},
		watch: {
			currentPage(val) {
		...
		computed: {
			...
			pageNumArr() {	//解决uni-app中存在v-for中循环数字从0开始
				var pageNumArr = [];
